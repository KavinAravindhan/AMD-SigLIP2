...
def get_augmentation_pipeline(trial):
    ...
    aug = A.Compose([
        ...
        A.GaussNoise(var_limit=(0, 1e-4), p=gauss_noise_prob),
...
class SigLIPModel(nn.Module):
    ...
    def forward(self, images, input_ids, attention_mask):
        ...
        # Classification: Use CLS token (first token) with dropout and 1152 dimensions
        cls_features = self.dropout(img_features[:, 0])  # Apply dropout
        cls_logits = self.cls_head(cls_features)  # (B, 1152) -> (B, 2)
...
import torch
import json
from {__name__.split('.')[0] if '.' in __name__ else 'train_optuna'} import SigLIPModel
